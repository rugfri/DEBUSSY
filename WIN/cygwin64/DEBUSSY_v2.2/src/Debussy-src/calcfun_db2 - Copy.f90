  ...
  do i =1, NDATA_W(CURR_SET)
   ...
   enddo
    do i =1, NDATA_W(CURR_SET)	
      gau_fwhm = four*dA_A*tan(degrees_to_radians/two*Obs_data_W(CURR_SET)%t2data(i))/degrees_to_radians 
      sig = gau_fwhm/(two*sqrt(two*log(two)))  
     if ((Obs_data_W(CURR_SET)%t2data(i)-(sig*three))>Obs_data_W(CURR_SET)%t2data(1) .and. &
       (Obs_data_W(CURR_SET)%t2data(i)+(sig*three))<Obs_data_W(CURR_SET)%t2data(NDATA_W(CURR_SET))) then 
