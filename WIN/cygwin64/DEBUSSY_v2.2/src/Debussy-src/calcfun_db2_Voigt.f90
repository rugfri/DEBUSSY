...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
 ...
 SUBROUTINE NANO_CALC_DBX(npin,pin,Curr_Str,Curr_Set)
   ...
   REAL(DP)       :: alp4(4)
   REAL(DP)       :: ln_n00,ln_wid,ln_n00_2nd,ln_wid_2nd, dA_A,dB_B,sig,gau_fwhm,lor_fwhm,nk,dxxx,vimax, area_55
   REAL(DP),allocatable :: var_irf(:), conv_ical(:)
   LOGICAL        :: refresh_Umat
   LOGICAL        :: refresh_Tmat
   character(132) :: funga 
   INTEGER(I4B)   :: klin(2),nupirf
...
!  
 ...
 !                                            s(n_2nd) = Omega_2nd-3*Delta_2nd/(2*n_2nd+1)
  ...
  ELSE IF (PARAPHAS(CURR_STR)%str_cod > 3 .and. PARAPHAS(CURR_STR)%str_cod <= 6) THEN
      ...
      IF (MICRO_FLAG(CURR_STR)==1) then 
           dA_A=PARAPHAS(CURR_STR)%nano_parcurrE(9)
            dB_B=PARAPHAS(CURR_STR)%nano_parcurrE(8)
      ENDIF
  ENDIF
  ...
  IF (MICRO_FLAG(CURR_STR)==1) then 
    ...
    dB_B=PARAPHAS(CURR_STR)%nano_parcurrE(8)
    IF (PRESENT(npin)) THEN
     PARAPHAS(CURR_STR)%nano_parcurrE(6:9) = (/Omega, Omega, dB_B,dA_A/)
     ELSE
...
!  dA_A=PARAPHAS(CURR_STR)%nano_parcurrE(9) 
! same variables used for IRF, to be changecd to use both!
   if (MICRO_FLAG(CURR_STR)==1) then 
    
     if (ALLOCATED(var_irf)) deallocate(var_irf)
     allocate(var_irf(NDATA_W(CURR_SET)))
     if (ALLOCATED(conv_ical)) deallocate(conv_ical)
     allocate(conv_ical(NDATA_W(CURR_SET)))


    do i =1, NDATA_W(CURR_SET)
    gau_fwhm = four*dA_A*tan(degrees_to_radians/two*Obs_data_W(CURR_SET)%t2data(i))/degrees_to_radians 
    lor_fwhm = four*dB_B*tan(degrees_to_radians/two*Obs_data_W(CURR_SET)%t2data(i))/degrees_to_radians
    
    var_irf = Voigt_F(x   = abs(OBS_DATA_W(CURR_SET)%t2data - OBS_DATA_W(CURR_SET)%t2data(i)), &
                           sig =gau_fwhm/(two*sqrt(two*log(two)))  , &
                           w   = lor_fwhm/two, &
                           epx = sceps_DP )
    
    vimax = maxval(var_irf)*s4eps_DP
    area_55 = sum(var_irf, mask=(var_irf > vimax) )
    nupirf = count(var_irf > vimax)
    if (nupirf==1) then
        conv_ical(i) = CALPHA_W(CURR_SET,CURR_STR)%vdata(i)
     else         
    conv_ical(i) = sum( var_irf * CALPHA_W(CURR_SET,CURR_STR)%vdata, mask=(var_irf > vimax) )/area_55 
    endif
     ! CALPHA_W(CURR_SET,CURR_STR)%vdata(i) = conv_ical(i)
    enddo
