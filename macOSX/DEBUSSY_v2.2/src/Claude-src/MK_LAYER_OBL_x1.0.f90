...
real(DP) :: parapar(9),wl_tt(2),smowid,coses(3),isinga,c_T,c_tau(2),c_tauMT(2),v001MT(3),MT_3dim(3,3), &
            pl_dis_vecCA(2),pl_dis_vecCB(2),L_dis_vecC(2)
...
real(DP), allocatable :: vertdissq(:,:,:,:),zvalstack(:,:,:,:)
...
print*, '                                         ------------------------------------'
print*, ' '
print*,'     Running MK_LAYER_OBL Program    '
print*, ' '
...
can1=ACCOMO(cos(degrees_to_radians*abcabgy(4)))
can2=ACCOMO(cos(degrees_to_radians*abcabgy(5)))
can3=ACCOMO(cos(degrees_to_radians*abcabgy(6)))

MT_3dim(1,:)=abcabgy(1)*[abcabgy(1),abcabgy(2)*can3,abcabgy(3)*can2]
MT_3dim(2,:)=abcabgy(2)*[abcabgy(1)*can3,abcabgy(2),abcabgy(3)*can1]
MT_3dim(3,:)=abcabgy(3)*[abcabgy(1)*can2,abcabgy(2)*can1,abcabgy(3)]
if (verbose) then
  print*,'########### MT matrix ############'
  print'("MT_",i4, 3f16.8)',1,MT_3dim(1,:)
  print'("MT_",i4, 3f16.8)',2,MT_3dim(2,:)
  print'("MT_",i4, 3f16.8)',3,MT_3dim(3,:)
endif  

DV=zero
v001MT = MT_3dim(1:3,3)
IF (ANY(ABS(abcabgy(4:5)-90.d0)<sceps_DP)) then
  DV=DVds_calc(abcabgy)
  c_T = DV(3,3)
  c2_T= v001MT(3)
  c_tau = DV(1:2,3)
  MTxy = matmul(transpose(planeDV),planeDV)
  c_tauMT = matmul(MTxy,c_tau)
ELSE
  DV(1:2,1:2)=planeDV
  DV(3,3) = c
  c_T = c
...
if (verbose) then
  print*,'########### DV matrix ############'
  print'("DV_",i4, 3f16.8)',1,DV(1,:)
  print'("DV_",i4, 3f16.8)',2,DV(2,:)
  print'("DV_",i4, 3f16.8)',3,DV(3,:)
  print*,'check:',maxval(abs(MTxy-MT_3dim(1:2,1:2)))
endif  
...
DO ipa=1,Celty(icel)%n_at_pair
  ...
  do ipz=1,ThePatVec(ipa)%num_Zs
    nmxyv=max(nmxyv,ThePatVec(ipa)%patbyz(ipz)%num_XYs)
  enddo
enddo
ALLOCATE(vertdissq(2,NCL2MK(2),nmzd,Celty(icel)%n_at_pair), &
         zvalstack(2,NCL2MK(2),nmzd,Celty(icel)%n_at_pair))
...
DO ipa=1,Celty(icel)%n_at_pair
  ...
  do jz=1,nzd
    zz0=ThePatVec(ipa)%patbyz(jz)%Zval
    zzmi=ThePatVec(ipa)%patbyz(jz)%Zmul_inv
    izzm=nint(zzmi)
    do js=1,izzm
      do lz=0,NCL2MK(2)-1
        cz=lz+updown(js)*zz0
        vertdissq(js,lz+1,jz,ipa)=(cz**2)*c2_T
        zvalstack(js,lz+1,jz,ipa) = cz * two
      enddo
    enddo
  enddo
enddo
...
!________prelim. :: sums of charges / excluding the z-multipl.
  ...
  if (verbose) print*,'Celty%* sizes: ',size(Celty(icel)%nat),size(Celty(icel)%xnat),size(Celty(icel)%ato,1),size(Celty(icel)%ato,2)
  if (verbose) print*,'Celty(1)%nat : ',Celty(icel)%nat
...
!            if (eqatpair.and.(.not.zerolay)) ooo_this=ooo_this*half !!!!!!!!!!!!!!

            cond_double = (isorigin .and. (.not.eqatpair))
            pl_dis_vecCA=L_dis_vecC+ThePatVec(ipa)%patbyz(jz)%xydist(1:2,jxy)
            pl_dis_vecCB=L_dis_vecC-ThePatVec(ipa)%patbyz(jz)%xydist(1:2,jxy)
...
!            plane_dis_sq1 = plane_dis_sqA + plane_dis_sqB
            plane_dis_sq1 = sum(pl_dis_vecCA*matmul(MTxy,pl_dis_vecCA))
            dpr_tauA = sum(pl_dis_vecCA*MT_3dim(3,1:2))
            
            do jC=1,NCL2MK(2)
              jzmm=-1
              if (jC==1) jzmm=1
              js=1; do jsz=1,jzmm,-2
                vv=vertdissq(js,jC,jz,ipa)
                uu=real(jsz,DP)*dpr_tauA*zvalstack(js,jC,jz,ipa)
                d0s=sqrt(max(zero,plane_dis_sq1+vv+uu)) 
...
!            plane_dis_sq2 = plane_dis_sqA - plane_dis_sqB
            plane_dis_sq2 = sum(pl_dis_vecCB*matmul(MTxy,pl_dis_vecCB))
            dpr_tauB = sum(pl_dis_vecCB*MT_3dim(3,1:2))
            ...
            do jC=1,NCL2MK(2) ! separation +1
              ...
              do jsz=1,jzmm,-2
                vv=vertdissq(js,jC,jz,ipa)
                uu=real(jsz,DP)*dpr_tauB*zvalstack(js,jC,jz,ipa)
                d0s=sqrt(max(zero,plane_dis_sq2+vv+uu))
                if (d0s<Celty(icel)%minallowdist(ipa)) then
                ! too short distance, set it to 0
                  do jc2=jC,NCL2MK(2)
                    zzm = Z_multipl(jC2,jC-1) * ooo_this 
